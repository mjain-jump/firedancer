...
/* test_incremental_release_partitions verifies the io-mode FAIL
   rollback of a snapshot writer's disk allocations: the whead attempt
   tracker records every acquired partition, and after the failed
   attempt's purge, fd_accdb_snapshot_worker_release_partitions returns
   the partitions to the pool with the shared counters
   (disk_current_bytes / disk_used_bytes / accounts_total) exactly
   restored, and a fresh attempt re-acquires the same disk space. */
static void
