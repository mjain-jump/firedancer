...
FD_STATIC_ASSERT( sizeof(fd_snapin_write_record_t)==16UL, write_record );
...
struct fd_snapin_tile {
  ...
  struct {
    uchar *                    buf;
    fd_snapin_write_record_t * records;
    ulong                      buf_used;
    ulong                      record_cnt;
  } writer;
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, 64UL,                              FD_SNAPIN_WRITE_BUF_SZ                                        );
  l = FD_LAYOUT_APPEND( l, alignof(fd_snapin_write_record_t), FD_SNAPIN_WRITE_RECORD_MAX*sizeof(fd_snapin_write_record_t) );
...
verify_slot_deltas_with_slot_history( fd_snapin_tile_t * ctx ) {
  if( FD_UNLIKELY( !ctx->lead.slot_history.captured ) ) {
    FD_LOG_WARNING(( "SlotHistory sysvar account was not present in the snapshot stream" ));
    return -1;
  }
  if( FD_UNLIKELY( !ctx->lead.slot_history.lamports || !ctx->lead.slot_history.data_len ) ) {
    FD_LOG_WARNING(( "SlotHistory sysvar account missing or empty" ));
    return -1;
  }
  if( FD_UNLIKELY( !fd_memeq( ctx->lead.slot_history.owner, fd_sysvar_owner_id.uc, sizeof(fd_pubkey_t) ) ) ) {
    FD_BASE58_ENCODE_32_BYTES( ctx->lead.slot_history.owner, owner_b58 );
    FD_LOG_WARNING(( "SlotHistory sysvar owner is invalid: %s != sysvar_owner_id", owner_b58 ));
    return -1;
  }

  fd_slot_history_view_t view[1];
  if( FD_UNLIKELY( !fd_sysvar_slot_history_view( view, ctx->lead.slot_history.buf, ctx->lead.slot_history.data_len ) ) ) {
...
txncache_staging_reset( fd_snapin_tile_t * ctx ) {
  ...
  ctx->lead.txncache_entries_len            = 0UL;
...
txncache_staging_slot_begin( fd_snapin_tile_t * ctx,
  ...
  if( FD_LIKELY( candidate_idx!=ULONG_MAX ) ) {
    ...
    ctx->lead.txncache_slots[ candidate_idx ].entry_base = ctx->lead.txncache_entries_len;
...
txncache_staging_compact( fd_snapin_tile_t * ctx ) {
  ...
  ulong cnt = ctx->lead.txncache_slots_len;
  for( ulong i=0UL; i<cnt; i++ ) { /* insertion sort by entry_base */
    ulong j=i;
    while( j>0UL && ctx->lead.txncache_slots[ order[ j-1UL ] ].entry_base>ctx->lead.txncache_slots[ i ].entry_base ) { order[ j ] = order[ j-1UL ]; j--; }
    order[ j ] = i;
  }
  ulong dst = 0UL;
  for( ulong k=0UL; k<cnt; k++ ) {
    txncache_staging_slot_t * s = &ctx->lead.txncache_slots[ order[ k ] ];
    FD_TEST( s->entry_base>=dst );
    if( s->entry_base!=dst ) memmove( &ctx->lead.txncache_entries[ dst ], &ctx->lead.txncache_entries[ s->entry_base ], s->entry_cnt*sizeof(fd_sstxncache_hash_t) );
    s->entry_base = dst;
    dst += s->entry_cnt;
  }
  FD_LOG_INFO(( "compacted staged txncache entries %lu -> %lu", ctx->lead.txncache_entries_len, dst ));
  ctx->lead.txncache_entries_len = dst;
}
...
txncache_staging_entry_add( fd_snapin_tile_t * ctx,
  ...
  if( FD_UNLIKELY( ctx->lead.txncache_entries_len>=ctx->lead.txncache_entries_max ) ) {
    txncache_staging_compact( ctx );
    if( FD_UNLIKELY( ctx->lead.txncache_entries_len>=ctx->lead.txncache_entries_max ) ) return -1;
  }
  ...
  FD_TEST( staging_slot->entry_base+staging_slot->entry_cnt==ctx->lead.txncache_entries_len );
  ...
  memcpy( ctx->lead.txncache_entries[ ctx->lead.txncache_entries_len ].txnhash, txnhash, sizeof(fd_sstxncache_hash_t) );
  ctx->lead.txncache_entries_len++;
...
filter_staged_groups( fd_snapin_tile_t *           ctx,
  ...
  ctx->lead.recent_groups_len = 0UL;
  for( ulong slot_idx=0UL; slot_idx<ctx->lead.txncache_slots_len; slot_idx++ ) {
    txncache_staging_slot_t const * staging_slot = &ctx->lead.txncache_slots[ slot_idx ];
    blockhash_group_t const *       groups       = &ctx->lead.blockhash_groups[ slot_idx*ctx->lead.txncache_max_groups_per_slot ];
...
populate_txncache( fd_snapin_tile_t *                     ctx,
  ...
  if( FD_UNLIKELY( !ctx->lead.blockhash_groups_cnt ) ) {
    ...
    if( FD_UNLIKELY( !ctx->lead.alpenglow || !ss.ele_cnt ) ) {
  ...
  for( ulong i=0UL; i<ctx->lead.recent_groups_len; i++ ) {
    ...
    for( ulong j=0UL; j<group->txncache_entry_cnt; j++ ) {
      fd_txncache_insert( ctx->lead.txncache, banks[ 0UL ].fork_id, banks[ group->chain_idx ].blockhash, entries[ j ].txnhash );
    }
  }
...
validate_capitalization( fd_snapin_tile_t * ctx ) {
  if( FD_UNLIKELY( ctx->lead.capitalization!=ctx->lead.manifest_capitalization ) ) {
    ...
    FD_LOG_WARNING(( "%s snapshot manifest capitalization %lu does not match computed capitalization %lu",
                     ctx->full?"full":"incr", ctx->lead.manifest_capitalization, ctx->lead.capitalization ));
    return -1;
  }
  return 0;
}
...
worker_snoop_winner( void * cb_ctx,
  ...
  if( FD_UNLIKELY( !memcmp( pubkey, fd_sysvar_slot_history_id.uc, 32UL ) ) ) {
    ...
    shmem->slot_history.slot       = batch->slot;
    shmem->slot_history.lamports   = lamports;
    shmem->slot_history.data_len   = data_len;
    shmem->slot_history.executable = batch->executables[ batch_idx ];
...
worker_commit_batch( fd_snapin_tile_t *              ctx,
  ...
  if( FD_UNLIKELY( fd_accdb_snapshot_write_batch_worker( ctx->accdb, fork_id, batch->cnt, batch->pubkeys, batch->slot, batch->lamports,
                                                         batch->data_lens, batch->executables, candidates,
                                                         ctx->stripe_locks, FD_SNAPIN_SHMEM_STRIPE_MSK,
                                                         batch->file_offsets, ctx->worker_metrics,
...
writer_flush( fd_snapin_tile_t * ctx ) {
  ...
  while( record_idx<ctx->writer.record_cnt ) {
    ...
    for( ulong i=0UL; i<cnt; i++ ) {
      ...
      FD_TEST( (ulong)record->slot==batch.slot );
...
writer_append_batch( fd_snapin_tile_t *   ctx,
  ...
  for( ulong i=0UL; i<cnt; i++ ) {
    ...
    record->slot       = (uint)slot;
    record->lamports   = lamports[ i ];
    record->batch_cnt  = (ushort)(i ? 0UL : cnt);
    record->executable = (ushort)executables[ i ];

    fd_accdb_disk_meta_t meta;
    fd_memcpy( meta.pubkey, pubkeys[ i ], 32UL );
    meta.size       = (uint)data_lens[ i ];
    meta.generation = 0U;
    fd_memcpy( meta.owner, owners[ i ], 32UL );
    fd_memcpy( ctx->writer.buf+ctx->writer.buf_used, meta.b, sizeof(meta) );
    fd_memcpy( ctx->writer.buf+ctx->writer.buf_used+sizeof(meta), datas[ i ], data_lens[ i ] );
    ctx->writer.buf_used += sizeof(meta)+data_lens[ i ];
  }
  ...
  for( ulong i=1UL; i<cnt; i++ ) {
    for( ulong j=0UL; j<i; j++ ) {
      if( FD_UNLIKELY( !memcmp( pubkeys[ j ], pubkeys[ i ], 32UL ) ) ) {
        FD_LOG_WARNING(( "corrupt snapshot: duplicate pubkey within a single batch (entries %lu and %lu, slot %lu)", j, i, batch_slot ));
        return -1;
      }
    }
  }

  return writer_append_batch( ctx, cnt, pubkeys, owners, datas, batch_slot, lamports, data_lens, executables );
}
...
worker_insert_one( fd_snapin_tile_t * ctx,
  ...
  return writer_append_batch( ctx, 1UL, pubkeys, owners, datas, slot, lamports_a, data_lens, executables );
}
  ...
  for(;;) {
    ...
    switch( res ) {
        ...
        while( bytes_remaining ) {
          ...
          if( FD_UNLIKELY( res<0 ) ) {
            ...
            if( FD_UNLIKELY( txncache_staging_entry_add( ctx, sd_result->entry->slot, sd_result->entry->txnhash ) ) ) {
              FD_LOG_WARNING(( "txncache entries overflow at slot %lu, max is %lu in total", sd_result->entry->slot, ctx->lead.txncache_entries_max ));
              transition_malformed( ctx, stem );
              return 0;
            }
          }
      ...
      case FD_SSPARSE_ADVANCE_ACCOUNT_HEADER:
        ...
        if( FD_UNLIKELY( ctx->lead.gui_out.idx!=ULONG_MAX
          ...
          ctx->lead.gui_config_acct_sz  = result->account_header.data_len;
      ...
      case FD_SSPARSE_ADVANCE_ACCOUNT_DATA:
        ...
        if( FD_UNLIKELY( ctx->lead.gui_config_acct_sz ) ) {
          uchar * acct = fd_chunk_to_laddr( ctx->lead.gui_out.mem, ctx->lead.gui_out.chunk );
          fd_memcpy( acct + ctx->lead.gui_config_acct_off, result->account_data.data, result->account_data.data_sz );
          ctx->lead.gui_config_acct_off += result->account_data.data_sz;

          if( FD_LIKELY( ctx->lead.gui_config_acct_off>=ctx->lead.gui_config_acct_sz ) ) {
            ctx->lead.gui_config_acct_sz = 0UL;
            if( FD_LIKELY( acct[ 0 ]==2UL ) ) {
              fd_stem_publish( stem, ctx->lead.gui_out.idx, 0UL, ctx->lead.gui_out.chunk, ctx->lead.gui_config_acct_off, 0UL, 0UL, 0UL );
              ctx->lead.gui_out.chunk = fd_dcache_compact_next( ctx->lead.gui_out.chunk, ctx->lead.gui_config_acct_off, ctx->lead.gui_out.chunk0, ctx->lead.gui_out.wmark );
              early_exit = 1;
            }
          }
        }
...
log_snoop_checksums( fd_snapin_tile_t * ctx ) {
  ...
  ulong feature_cs = fd_hash( 0xFEA7UL, ctx->lead.feature_snoop, sizeof(fd_feature_snoop_t) );
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  FD_TEST( accdb_shmem );
  if( FD_UNLIKELY( fd_accdb_shmem_partition_sz( accdb_shmem )<FD_SNAPIN_WRITE_BUF_SZ ) ) {
    FD_LOG_ERR(( "accdb partition size must be at least %lu bytes", FD_SNAPIN_WRITE_BUF_SZ ));
  }
  ...
  if( FD_UNLIKELY( is_lead( ctx ) ) ) {
    ...
    ctx->lead.bank = fd_banks_init_bank( ctx->banks );
