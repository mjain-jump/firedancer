...
/* The snapin tile role is its kind_id: tile 0 is the coordinator, tiles
   1..N are accdb workers. */
...
/* Per-worker staging buffer coalescing the packed disk records of
...
/* Upper bound on a single tar entry's claimed size.  Tar sizes are
   attacker controlled (the OLDGNU base-256 encoding expresses up to
   2^64-1), and the byte-coverage protocol does arithmetic on
   stream_offset + align_up(size, 512): an absurd size must be rejected
   as a malformed snapshot rather than overflow the coverage math.  One
   TiB is far above any real appendvec or manifest (entire mainnet
   snapshots decompress to well under this) while keeping every offset
   comfortably inside 2^63. */

#define FD_SNAPIN_MAX_TAR_ENTRY_SZ (1UL<<40)
...
/* Write-behind: workers kick async writeback of their flushed records
   ...
   throttled with coarse (up to ~100 ms) sleeps -- and a sleeping worker
   freezes its lane fseqs (worker fseq == scan position), which convoys
   the ENTIRE pipe (measured at eight workers: raw intake collapsed from
   ...
   default) to preserve the page-cache elasticity that low worker counts
   rely on. */
...
/* Worker count at and above which write-behind is engaged.  Below it,
...
/* Coordinator ring-publish budget per stem iteration.  Everything the
   coordinator publishes in ONE stem iteration must fit in STEM_BURST
   (== FD_SNAPIN_IO_BURST): stem checks credits once per iteration and
   then runs BOTH after_credit and (at most) one frag callback, so their
   publishes stack.  io_pub_cnt counts ring publishes per iteration
   (reset at the top of the coordinator's after_credit, so an idle
   watermark broadcast that fires there is inside the budget) and the
   data parse loop stops taking new steps once it reaches the cap.  The
   worst iteration enters its last budgeted parse step at cap-1 ring
   publishes and then chains the two broadcasts a single callback can
   still emit -- an EOS at stream end followed by an ABORT when the
   trailing bytes fail to parse -- plus one manifest and one snapct
   control publish:

     cap-1 + 2*FD_SNAPIN_WORKER_MAX (EOS+ABORT broadcasts) + 2

   The post-frag watermark cadence only fires with a full broadcast of
   budget left, so it never adds to the worst case.  The static assert
   keeps the bound in sync with FD_SNAPIN_IO_BURST (with one slot of
   slack). */

#define FD_SNAPIN_IO_PUB_CAP (14UL)

FD_STATIC_ASSERT( FD_SNAPIN_IO_PUB_CAP-1UL+2UL*FD_SNAPIN_WORKER_MAX+2UL<=FD_SNAPIN_IO_BURST, snapin_io_pub_cap_burst );
...
/* Tar-boundary sharding.  With workers attached the snapin tile has two
   roles:

     coordinator: the single-tile snapin state machine, plus the ssparse
       appendvec passthrough.  It never parses individual accounts, but
       instead assigns whole appendvecs to workers at tar-header parse
       time (ASSIGN on the owning worker's snapin_io ring) and publishes
       a scalar stream-offset coverage watermark ("assignments complete
       below X") to every worker.  It still fully parses and verifies the
       manifest and status cache, owns the control protocol with snapct
       (forwarding each control only after all workers acked it on their
       own lane barriers), and merges the workers' snoop staging at end
       of load.

     worker: a fused parse+insert+write loader.  It consumes the same
       snapdc_in lanes (full reliable consumer with the same
...
struct fd_snapin_tile {
  ...
  ulong io_pub_cnt; /* snapin_io publishes so far in the current stem
                       iteration (reset at the top of after_credit), to
                       respect STEM_BURST.  A frag packed with tiny
                       appendvecs can otherwise publish more than
                       STEM_BURST jobs; see FD_SNAPIN_IO_PUB_CAP. */
  ...
  /* Coordinator: partitions abandoned by failed attempts (gathered from
     the workers' snoop staging at FAIL-ack fold), released back to the
  ...
  /* Worker: lamports folded into the coordinator's capitalization from
     the FINI ack. */
  struct {
  ...
  uint * stripe_locks;                             /* shared accdb chain locks */
  ...
  struct {                                         /* coverage instrumentation, logged at FINI */
...
/* Coordinator: publish one job on a worker's ring.  The caller supplies
   the kind-specific fields; the current attempt's generation is stamped
   here, so no published job can escape the generation discipline. */

static void
publish_io_job( fd_snapin_tile_t *         ctx,
                fd_stem_context_t *        stem,
                ulong                      worker_idx,
                fd_snapin_io_job_t const * job_in ) {
  fd_snapin_out_link_t * out = &ctx->io_out[ worker_idx ];
  fd_snapin_io_job_t * job = fd_chunk_to_laddr( out->mem, out->chunk );
  *job            = *job_in;
  job->generation = ctx->generation;
  fd_stem_publish( stem, out->idx, job->kind, out->chunk, sizeof(fd_snapin_io_job_t), 0UL, 0UL, 0UL );
  io_out_advance( ctx, worker_idx );
  ctx->io_pub_cnt++;
  ctx->io_jobs_since_credit++;
}

/* Coordinator: publish a non-ASSIGN marker (WATERMARK/EOS/ABORT) to
   every worker ring.  fork_id is only meaningful on an ASSIGN. */

static void
broadcast_io_marker( fd_snapin_tile_t *  ctx,
                     fd_stem_context_t * stem,
                     ulong               kind,
                     ulong               covered_until ) {
  fd_snapin_io_job_t marker = { .kind = kind, .covered_until = covered_until, .fork_id = ULONG_MAX };
  for( ulong worker_idx=0UL; worker_idx<ctx->worker_cnt; worker_idx++ ) {
    publish_io_job( ctx, stem, worker_idx, &marker );
  }
}
...
publish_watermarks( fd_snapin_tile_t *  ctx,
                    fd_stem_context_t * stem ) {
  broadcast_io_marker( ctx, stem, FD_SNAPIN_IO_KIND_WATERMARK, ctx->covered_until );
...
publish_abort( fd_snapin_tile_t *  ctx,
  ...
  broadcast_io_marker( ctx, stem, FD_SNAPIN_IO_KIND_ABORT, 0UL );
}
...
/* An account body that straddles decompressed-chunk boundaries arrives
   as several ACCOUNT_DATA results, so a snoop that needs the whole body
   reassembles it in a window armed at ACCOUNT_HEADER.  reasm_append
   copies as much of [data,data+sz) as the window still needs and
   returns 1 once it is complete. */

static inline int
reasm_append( uchar *       buf,
              ulong *       write_pos,
              ulong         need,
              uchar const * data,
              ulong         sz ) {
  ulong copy_sz = fd_ulong_min( sz, need-*write_pos );
  fd_memcpy( buf+*write_pos, data, copy_sz );
  *write_pos += copy_sz;
  return *write_pos==need;
}
...
process_account_batch( fd_snapin_tile_t *            ctx,
  ...
  return fd_int_if( write_account_batch( ctx, cnt, pubkeys, batch_slot, lamports, data_lens, executables )<0, -1, 0 );
}
...
process_account_data( fd_snapin_tile_t *            ctx,
  ...
  if( FD_UNLIKELY( ctx->slot_history.capturing ) &&
      reasm_append( ctx->slot_history.buf, &ctx->slot_history.write_pos,
                    ctx->slot_history.data_len, data, data_sz ) ) {
    ctx->slot_history.captured  = 1;
    ctx->slot_history.capturing = 0;
  }

  if( FD_UNLIKELY( ctx->feature_reasm.capturing ) &&
      reasm_append( ctx->feature_reasm.buf, &ctx->feature_reasm.write_pos,
                    ctx->feature_reasm.need, data, data_sz ) ) {
    fd_feature_snoop_account( ctx->feature_snoop, &ctx->feature_reasm.pubkey,
                              ctx->feature_reasm.lamports, ctx->feature_reasm.owner,
  ...
  if( FD_UNLIKELY( ctx->stake_reasm.capturing ) &&
      reasm_append( ctx->stake_reasm.buf, &ctx->stake_reasm.write_pos,
                    sizeof(ctx->stake_reasm.buf), data, data_sz ) ) {
...
handle_data_frag( fd_snapin_tile_t *  ctx,
  ...
  for(;;) {
    ...
    /* Cap ring publishes per iteration (one frag holds up to ~127 tar
       headers, each of which publishes); the unconsumed tail of the frag
       is reprocessed with fresh credits.  io_pub_cnt was reset by this
       iteration's after_credit, so an idle watermark broadcast that ran
       there is included in the budget. */
    if( FD_UNLIKELY( ctx->io_enabled && ctx->io_pub_cnt>=FD_SNAPIN_IO_PUB_CAP ) ) break;
    ...
    switch( res ) {
      ...
      case FD_SSPARSE_ADVANCE_APPENDVEC: {
        ...
        if( FD_UNLIKELY( body_sz>FD_SNAPIN_MAX_TAR_ENTRY_SZ ) ) {
          FD_LOG_WARNING(( "oversized appendvec tar entry (%lu bytes) in snapshot stream", body_sz ));
          transition_malformed( ctx, stem );
          return 0;
        }
        ulong w = 0UL;
        ...
        publish_io_job( ctx, stem, w, &(fd_snapin_io_job_t){
            .kind          = FD_SNAPIN_IO_KIND_ASSIGN,
            .appendvec_idx = ctx->appendvec_seq++,
            .slot          = result->appendvec.slot,
            .body_off      = body_off,
            .body_sz       = body_sz,
            .covered_until = ctx->covered_until,
            .fork_id       = assign_fork } );
      ...
      case FD_SSPARSE_ADVANCE_REGION: {
        ...
        if( FD_UNLIKELY( result->region.data_sz>FD_SNAPIN_MAX_TAR_ENTRY_SZ ) ) {
          FD_LOG_WARNING(( "oversized tar entry (%lu bytes) in snapshot stream", result->region.data_sz ));
          transition_malformed( ctx, stem );
          return 0;
        }
        ulong body_off = ctx->stream_cursor + result->bytes_consumed;
      ...
      case FD_SSPARSE_ADVANCE_ACCOUNT_HEADER:
        ...
        /* TODO(parallel snapshot load): with workers attached the
           coordinator runs in appendvec passthrough and never sees
           account bytes, so this ConfigProgram capture (validator
           name/icon for the GUI) never fires and snapin_gui stays
           silent until gossip/replay refresh the info.  Fixing it needs
           the snoop-staging pattern: per-worker logs of ConfigProgram
           account payloads (<= FD_GUI_CONFIG_PARSE_MAX_VALID_ACCT_SZ
           each, keyed (appendvec_idx, record_idx) like the stake log),
           k-way merged by the coordinator at NEXT/DONE and drained to
           snapin_gui at a publish-budgeted pace from after_credit. */
      ...
      case FD_SSPARSE_ADVANCE_DONE:
        if( FD_UNLIKELY( ctx->io_enabled ) ) broadcast_io_marker( ctx, stem, FD_SNAPIN_IO_KIND_EOS, 0UL );
...
/* Worker staging buffer: buffered pwrites into the worker's own accdb
   partitions.  flush_off is explicit because per-worker offsets are only
...
/* Returns 0 on success, -1 if the stake log is full (the caller must
   fail the attempt). */
...
worker_snoop_stake( fd_snapin_tile_t * ctx,
                    ...
                    ulong              rec_idx ) {
  fd_stake_state_t const * stake_state = fd_stake_state_view( data, data_sz );
  if( FD_UNLIKELY( !stake_state || stake_state->stake_type!=FD_STAKE_STATE_STAKE ) ) return 0;

  fd_delegation_t const * delegation = &stake_state->stake.stake.delegation;
  if( FD_UNLIKELY( ( delegation->activation_epoch!=ULONG_MAX &&
                    delegation->activation_epoch>=(ulong)USHORT_MAX ) ||
                   ( delegation->deactivation_epoch!=ULONG_MAX &&
                    delegation->deactivation_epoch>=(ulong)USHORT_MAX ) ) ) return 0;

  fd_snapio_worker_snoop_t * ws = ctx->my_snoop;
  if( FD_UNLIKELY( ws->stake_cnt>=ctx->stake_log_max ) ) {
    /* Untrusted input can carry any number of stake-program accounts:
       fail the attempt (snapct retries / aborts) instead of taking the
       validator down. */
    FD_LOG_WARNING(( "stake snoop log overflow (worker %lu, max %lu entries): failing the snapshot load attempt",
                     ctx->worker_idx, ctx->stake_log_max ));
    return -1;
  }
...
/* worker_cov_advance advances the worker's coverage watermark and
   samples the coverage lag (the runway ahead of the scan cursor, which
   is what absorbs a worker's write stalls without stalling the pipe). */
...
worker_process_account_batch( fd_snapin_tile_t *            ctx,
  ...
  for( ulong i=0UL; i<cnt; i++ ) {
    ...
    if( FD_UNLIKELY( lamports[ i ] &&
                     !memcmp( e+64UL, &fd_solana_stake_program_id, sizeof(fd_pubkey_t) ) ) ) {
      if( FD_UNLIKELY( 0!=worker_snoop_stake( ctx, pubkeys[ i ], lamports[ i ], data_lens[ i ], e+136UL, data_lens[ i ],
                                              ctx->cur_av.appendvec_idx, rec ) ) ) return -1;
    }
  }
  ctx->rec_idx += cnt;
...
worker_process_account_data( fd_snapin_tile_t *            ctx,
  ...
  if( FD_UNLIKELY( ctx->sh_reasm.capturing ) &&
      reasm_append( ctx->my_snoop->slot_history.buf, &ctx->sh_reasm.write_pos,
                    ctx->sh_reasm.data_len, data, data_sz ) ) {
    ctx->my_snoop->slot_history.captured = 1;
    ctx->sh_reasm.capturing              = 0;
  }
  ...
  if( FD_UNLIKELY( ctx->stake_reasm.capturing ) &&
      reasm_append( ctx->stake_reasm.buf, &ctx->stake_reasm.write_pos,
                    sizeof(ctx->stake_reasm.buf), data, data_sz ) ) {
    ctx->stake_reasm.capturing = 0;
    if( FD_UNLIKELY( 0!=worker_snoop_stake( ctx, ctx->stake_reasm.pubkey.uc, ctx->stake_reasm.lamports,
                                            ctx->stake_reasm.data_len, ctx->stake_reasm.buf, sizeof(ctx->stake_reasm.buf),
                                            ctx->reasm_av_idx, ctx->reasm_rec_idx ) ) ) return -1;
  }
  return 0;
}
  ...
  for(;;) {
    ...
    switch( res ) {
      ...
      case FD_SSPARSE_ADVANCE_ACCOUNT_HEADER:
        ...
        break;
      case FD_SSPARSE_ADVANCE_ACCOUNT_DATA:
        fail = worker_process_account_data( ctx, result )<0;
        break;
      case FD_SSPARSE_ADVANCE_ACCOUNT_BATCH:
...
worker_try_complete_fini( fd_snapin_tile_t *  ctx,
  ...
  /* Coverage instrumentation: if the holds are frequent and the lag is
     small the pipe is flow-latency bound, not work bound. */
  ...
  /* The FINI ack just handed the attempt's counters to the coordinator,
     which folds them into its own gauges: zero this worker's account
     gauges so dashboards summing over all snapin tiles do not count the
     load twice while the fold and the fresh gauges coexist. */
  ctx->metrics.accounts_loaded   = 0UL;
  ctx->metrics.accounts_replaced = 0UL;
  ctx->metrics.accounts_ignored  = 0UL;
}
...
worker_handle_job( fd_snapin_tile_t *  ctx,
  ...
  switch( job->kind ) {
    ...
    case FD_SNAPIN_IO_KIND_ASSIGN: {
      ...
      /* The coordinator bounds tar entry sizes (FD_SNAPIN_MAX_TAR_ENTRY_SZ)
         before assigning, so the coverage arithmetic cannot wrap; treat a
         wrapping extent as a protocol violation anyway. */
      ulong end = job->body_off + fd_ulong_align_up( job->body_sz, 512UL );
      if( FD_UNLIKELY( end<job->body_off ) ) {
        worker_enter_error( ctx, stem, EPROTO, 0 );
        return 0;
      }
...
worker_handle_control_frag( fd_snapin_tile_t *  ctx,
  ...
  switch( sig ) {
    case FD_SNAPSHOT_MSG_CTRL_INIT_FULL: {
      ...
      ctx->my_snoop->fail_partition_cnt = 0UL; /* already consumed at the FAIL-ack fold */
    ...
    case FD_SNAPSHOT_MSG_CTRL_FAIL: {
      FD_TEST( ctx->state!=FD_SNAPSHOT_STATE_SHUTDOWN );
      ctx->state = FD_SNAPSHOT_STATE_IDLE;
      /* Close the final open partition (stamps its write_offset and
         books the tail slack, exactly like a rotation would) and
         publish this attempt's partition list before dropping the
         tracker: the coordinator releases the partitions once the
         index purge completed -- at the next INIT_INCR for a failed
         incremental attempt, or implicitly via fd_accdb_reset at the
         retry's INIT_FULL for a failed full one.  The staging buffer
         and private write head are then simply forgotten. */
      fd_accdb_snapshot_worker_close( ctx->accdb, &ctx->whead );
      /* Fold the attempt's buffered shared-counter deltas
         (disk_used_bytes, accounts_total) BEFORE they are discarded:
         the coordinator's purge subtracts per unlinked entry, so
         discarding the additions would leave the shared counters
         net-negative by the attempt's contribution. */
      fd_accdb_snapshot_flush_worker_metrics( ctx->accdb, ctx->worker_metrics );
...
static void
coordinator_merge_snoops( fd_snapin_tile_t * ctx ) {
  ...
  for( ulong w=0UL; w<ctx->worker_cnt; w++ ) {
    ...
    int wins;
    if(      ws->slot_history.slot         !=bs->slot_history.slot          ) wins = ws->slot_history.slot         >bs->slot_history.slot;
    else if( ws->slot_history.appendvec_idx!=bs->slot_history.appendvec_idx ) wins = ws->slot_history.appendvec_idx>bs->slot_history.appendvec_idx;
    else                                                                     wins = ws->slot_history.record_idx   >bs->slot_history.record_idx;
    if( wins ) best = w;
  }
...
/* Order-independent checksums of the merged snoop state.  The parallel
...
/* Appendvec count and size distribution.  An appendvec is parseable
   only sequentially by its owning worker, so any appendvec larger than
   the in-flight lane window forces the whole pipe down to one worker's
...
coordinator_finish_failed_attempt( fd_snapin_tile_t * ctx ) {
  /* Gather the workers' per-attempt partition lists (quiescent: every
     worker stamped its list before its FAIL ack, and all acks are in).
     They are released once the purge below has completed: at the next
     INIT_INCR for a failed incremental attempt, or implicitly by
     fd_accdb_reset at the retry's INIT_FULL for a failed full one. */
  if( FD_UNLIKELY( ctx->io_enabled ) ) {
    for( ulong w=0UL; w<ctx->worker_cnt; w++ ) {
      fd_snapio_worker_snoop_t * ws = ctx->snoops[ w ];
  ...
  if( FD_LIKELY( ctx->init_completed ) ) {
    if( ctx->full ) {
      ...
      /* revert_whead is whead[0]-based; in io mode NEXT never saved it
         ...
         partitions sit in the doomed list instead, awaiting the purge
         (which unlinks the index entries referencing them). */
  ...
  switch( sig ) {
    ...
    case FD_SNAPSHOT_MSG_CTRL_INIT_INCR: {
      ...
      if( sig==FD_SNAPSHOT_MSG_CTRL_INIT_FULL ) {
        ...
        fd_accdb_reset( ctx->accdb );
        /* The reset released every partition, including any doomed ones
           from previously failed attempts. */
        ...
        /* attach_child waited for any pending background command, so a
           previously failed incremental attempt's purge has completed
           and no index entry references its workers' partitions
           anymore: release them back to the partition pool before this
           attempt starts allocating. */
    ...
    case FD_SNAPSHOT_MSG_CTRL_NEXT: {
      ...
      /* save_whead is whead[0]-based and meaningless under explicit
         per-worker offsets.  In io mode, incremental failure recovery
         instead purges the incremental fork and releases the workers'
         per-attempt partitions (see coordinator_finish_failed_attempt
         and the INIT_INCR handler). */
...
returnable_frag( fd_snapin_tile_t *  ctx,
  ...
  /* Watermark cadence: broadcast the coverage watermark every
     ...
     after_credit path covers assignment-less stretches.  Only fires
     with a full broadcast of publish budget left this iteration (see
     FD_SNAPIN_IO_PUB_CAP); a typical frag publishes far below the cap,
     and a skipped broadcast is retried on the next consumed frag or
     from an idle after_credit. */
  if( FD_UNLIKELY( ctx->io_enabled && !reprocess && in_idx<ctx->lane_cnt &&
    ...
    if( FD_UNLIKELY( ctx->io_watermark_dirty &&
                     ...
                     ctx->pending_worker_control==ULONG_MAX &&
                     ctx->io_pub_cnt+ctx->worker_cnt<=FD_SNAPIN_IO_PUB_CAP ) ) {
      publish_watermarks( ctx, stem );
    }
  }
...
after_credit( fd_snapin_tile_t *  ctx,
  ...
  /* Ring-publish budget is per stem iteration (one after_credit plus at
     most one frag callback share one STEM_BURST credit check); this is
     the start of the iteration's publishes.  See FD_SNAPIN_IO_PUB_CAP. */
  ctx->io_pub_cnt = 0UL;
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  if( FD_UNLIKELY( is_accdb_worker( ctx ) ) ) {
    ...
    /* Track this attempt's partitions straight into the shared staging,
       so a failed attempt's list only needs its count stamped before
       the FAIL ack. */
    ...
    ctx->wb_window  = snoop_hdr->worker_cnt>=FD_SNAPIN_WB_MIN_WORKERS
                    ? FD_SNAPIN_WB_TOTAL_WINDOW/snoop_hdr->worker_cnt : 0UL;
