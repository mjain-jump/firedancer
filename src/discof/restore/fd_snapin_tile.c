...
/* For a transaction to be valid to be inserted into the txncache, it
   ...
   of entries (2x the transactions per slot,
   config->limits.max_txn_per_slot). */
...
/* Read shared totals after all FINI acks. */
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  if( FD_UNLIKELY( is_lead( ctx ) ) ) {
    ...
    ctx->lead.txncache_entries_max         = FD_TXNCACHE_MAX_SLOT_DELTAS*2UL*tile->snapin.max_txn_per_slot;
