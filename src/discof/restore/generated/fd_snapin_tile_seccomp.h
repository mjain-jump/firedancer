...
static const uint sock_filter_policy_fd_snapin_tile_instr_cnt = 49;

static void populate_sock_filter_policy_fd_snapin_tile( ulong out_cnt, struct sock_filter out[ static 49 ], uint logfile_fd, uint accounts_fd, uint snapshot_fd ) {
  FD_TEST( out_cnt >= 49 );
  struct sock_filter filter[49] = {
    ...
    /* check pwrite64 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pwrite64, /* check_pwrite64 */ 25, 0 ),
    /* check preadv2 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_preadv2, /* check_preadv2 */ 28, 0 ),
    /* check sync_file_range */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_sync_file_range, /* check_sync_file_range */ 31, 0 ),
    /* check pread64 */
