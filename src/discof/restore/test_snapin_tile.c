...
static uchar test_coord_snoop_mem[ FD_SNAPIN_WORKER_MAX ][ sizeof(fd_snapio_worker_snoop_t) ] __attribute__((aligned(64)));
...
coordinator_io_ctx_init_n( fd_snapin_tile_t * ctx,
  ...
  fd_memset( test_coord_snoop_mem, 0, sizeof(test_coord_snoop_mem) );
  for( ulong w=0UL; w<worker_cnt; w++ ) ctx->snoops[ w ] = (fd_snapio_worker_snoop_t *)test_coord_snoop_mem[ w ];
  ctx->generation = 1UL;
