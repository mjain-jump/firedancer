...
static void
test_storage_error_retries( void ) {
  for( int incremental=0; incremental<2; incremental++ ) {
    ...
    ulong checkpoint = incremental ? 1000UL : 0UL;
    test_file_off = checkpoint;
    cl->ctx[ 0 ].lead.recovery.accdb_metadata.whead_val = checkpoint;
    ulong init_sig = incremental ? FD_SNAPSHOT_MSG_CTRL_INIT_INCR : FD_SNAPSHOT_MSG_CTRL_INIT_FULL;
    cluster_barrier( cl, init_sig );
    ulong owner[ TEST_AV_MAX ];
    cluster_stream( cl, TEST_ORDER_ROUND_ROBIN, owner );
    ...
    ctx->staged.slot       = 42UL;
    ctx->staged.lamports   = 1UL;
    ctx->staged.data_len   = 1UL;
    ctx->staged.executable = 0;
    fd_memcpy( ctx->staged.pubkey, pubkey,        32UL );
    fd_memcpy( ctx->staged.owner,  account_owner, 32UL );
    fd_memcpy( ctx->staged.data,   data,            1UL );
    FD_TEST( !writer_append_staged( ctx ) );
    test_pwrite_push( -1L, ENOSPC );

    ulong entry_sz = sizeof(fd_accdb_disk_meta_t)+1UL;
    ulong pub0 = test_pub_cnt;
    cluster_barrier( cl, FD_SNAPSHOT_MSG_CTRL_FINI );
    FD_TEST( ctx->state==FD_SNAPSHOT_STATE_ERROR );
    FD_TEST( test_pub_cnt==pub0+1UL && test_pub_sig[ pub0 ]==FD_SNAPSHOT_MSG_CTRL_ERROR );
    FD_TEST( test_file_off==checkpoint+entry_sz );
    ...
    FD_TEST( ctx->state==FD_SNAPSHOT_STATE_PROCESSING );
    FD_TEST( !ctx->writer.buf_used );
    FD_TEST( test_accdb_purge_cnt==(ulong)incremental );
    FD_TEST( test_accdb_revert_whead_cnt==(ulong)incremental );
...
test_writer_short_write_and_eintr( void ) {
  ...
  FD_TEST( writer_pwrite( ctx, data, 4UL, 0UL )==1 );
  FD_TEST( test_pwrite_call_cnt==2UL );
  FD_TEST( test_pwrite_off[1]==2UL && test_pwrite_data[1][0]==3U );
