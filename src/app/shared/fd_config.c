...
fd_config_validatef( fd_configf_t const * config ) {
  ...
  if( FD_UNLIKELY( !config->layout.snapdc_tile_count || config->layout.snapdc_tile_count>FD_SNAPIN_LANE_MAX ) ) {
    FD_LOG_ERR(( "`layout.snapdc_tile_count` must be in [1,%lu] (each snapshot decompressor lane is tracked by every snapshot loader tile)", FD_SNAPIN_LANE_MAX ));
  }
