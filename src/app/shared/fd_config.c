...
fd_config_validatef( fd_configf_t const * config ) {
  ...
  if( FD_UNLIKELY( config->layout.snapin_tile_count<2U ||
                   config->layout.snapin_tile_count>FD_SNAPIN_TILE_MAX ||
                   !fd_ulong_is_pow2( config->layout.snapin_tile_count-1UL ) ) ) {
    FD_LOG_ERR(( "`layout.snapin_tile_count` must be a snapshot coordinator plus a power of two count of loader tiles: one of 2, 3, 5, or 9" ));
  }
