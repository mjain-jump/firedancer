...
fd_gui_run_boot_progress( fd_gui_t * gui, long now ) {
  ...
  /* snapin tile 0 coordinates the load; when more snapin tiles exist
     they are the accdb workers doing the account inserts (and there is
     no snapwr tile: the workers write the accounts file themselves). */
  ...
  switch ( gui->summary.boot_progress.phase ) {
    ...
    case FD_GUI_BOOT_PROGRESS_TYPE_LOADING_INCREMENTAL_SNAPSHOT: {
      ...
      /* Sum over all snapin tiles: with parallel loading only the
         workers count inserted accounts (the coordinator's gauge stays
         0), and with a lone snapin tile only it does. */
      ...
      /* Without a snapwr tile the snapin workers write account data to
